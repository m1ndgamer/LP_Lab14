...
namespace MFST
{
	...
	Mfst::Mfst(LEX::Lex plex, GRB::Greibach pgrebach) //конструктор
	{
		...
		for (int k = 0; k < lenta_size; k++)
			lenta[k] = TS(lex.lextable->table[k].lexem);	// заполнение массива терминалами
		lenta_position = 0;
	...
	char* Mfst::getDiagnosis(short n, char* buf) //вывод диагностики
	{
		char* rc = (char*)"";
		int errid = 0;
		int lpos = -1;
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
		{
			...
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: строка %d, %s", err.id, lex.lextable->table[lpos].lineNumber, err.message);
			rc = buf;
		};
		return rc;
	};
